...
from worker_funcs import bg_job
...
async def get_game_state(game_id):
    ...
    read_db_str = __get_read_db_str()
    read_db = databases.Database(read_db_str)
    await read_db.connect()
    app.logger.info(f"read_db: {str(read_db_str)}")
...
async def check_guess(data, game_id):
    ...
    if info[1] > 0:
        ...
        # find the callbackURL
        query = "SELECT url FROM callbackURLs;"
        url = await db.fetch_val(query=query)
